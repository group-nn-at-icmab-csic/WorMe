function [nom_im_carpeta_coincident] = compare_BW_coincident_infolder(BW_aillar, nom_BW_modif, carpeta_imatges_BW_totes)
...
[theFiles_carpetaBB] = read_imgs_folder_structure(carpeta_imatges_BW_totes);
...
for cada_imatge = 1:length(theFiles_carpetaBB)
    ...
    if strcmp(nom_BW_modif, nom_carpeta_files_original)
        ...
        [llist_BW_identif, BW_bwselect_igual_punts, imatge_montatge_sortida] = compare_BW_coincident_object(imatge_a_llegir, BW_aillar);
...
if isempty(cont_contrl_im_coincid)
    ...
    if cont_contrl_im_coincid > 1
        ...
        error("f'compare_BW_coincident_infolder': More than a compared object")
    end
end
