function imageArray_fintot = superpose_semiBW_RGB(imageArray, BW_semi)

% Given a normal 3D RGB image (imageArray), and another binary 3D image (BW_semi), 
...
if size(BW_semi) ~= size(imageArray)
    ...
    error("Error in f'superpose_semiBW_RGB': Images different size")
end
