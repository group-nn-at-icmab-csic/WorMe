function [formatSpec] = tipologia_formatspec(cell_dades_totals)

% Determina el formatSpec a l'hora de fer un fprintf, quan es crea un
% arxiu, de les dades d'una cell.
%
%
% See also
% write_dades_cell
...
formatSpec = "";
for n_cell = 1:length(cell_dades_totals)
    % cell_dades_totals{n_cell}
    if isequal(string(class(cell_dades_totals{n_cell})), "double")
        formatSpec = strcat(formatSpec, "%f");
    elseif isequal(string(class(cell_dades_totals{n_cell})), "string")
